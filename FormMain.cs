...
namespace WindowsFormsApp1
{
    ...
    public partial class FormMain : UIForm
    {
        ...
        public XGNetSDK.NET_DVR_USER_LOGIN_INFO struLogInfo;//用户登录信息结构体
        XGNetSDK.LOGINRESULTCALLBACK LoginCallBack = null;//登录结果回调函数
        ...
        public XGNetSDK.NET_DVR_DEVICEINFO_V40 DeviceInfo;//设备信息结构体
        //XGNetSDK.REALDATACALLBACK RealData = null;
        //XGNetSDK.REALDATACALLBACK RealData1 = null;

        public static Int32[] m_UserIDs;//设备ID，每台设备有一个ID
        XGNetSDK.REALDATACALLBACK[] RealDatas;//实时图像数据委托函数
        private long[] m_RealHandles;//实时句柄
        GetTempUtil getTempUtil = new GetTempUtil();//获取温度工具类
        ...
        public FormMain()
        {
            ...
            //LoginDevice(0, "192.168.2.9", "admin", "admin", "8903", cbLoginCallBack);//登录设备
            //Thread.Sleep(100);
            ...
            //LoginDevice(1, "192.168.2.10", "admin", "admin", "8903", cbLoginCallBack1);
            //Thread.Sleep(100);
        ...
        {
            int cameraNum = Globals.systemParam.cameraNum;//设备数量
            pics = new PictureBox[cameraNum * 2];//定义显示图像控件，每个设备显示可见光和红外图像。
            labels = new TransparentLabel[cameraNum * 2];//显示控件说明Label

            //pixUIPanel = new UIPanel();

            m_UserIDs = new Int32[cameraNum];//每个设备有一个userID
            m_RealHandles = new long[cameraNum * 2];//每个设备有两个实时显示句柄，可见光和红外
            RealDatas = new XGNetSDK.REALDATACALLBACK[cameraNum * 2];//每个设备有两个实时数据回调函数
            ...
            getTempUtil.init(384, 288);//初始化获取温度工具类
        ...
        /// <param name="e"></param>
        void Pics0_Paint(object sender, PaintEventArgs e)
        {
            if ((int)pics[0].Tag == 1)
            {
                ControlPaint.DrawBorder(e.Graphics, pics[0].ClientRectangle, PIC_CLICKED_COLOR, ButtonBorderStyle.Solid);
                //ControlPaint.DrawBorder(e.Graphics, pics[0].ClientRectangle, clickedColor,10, ButtonBorderStyle.Solid, clickedColor, 1, ButtonBorderStyle.Solid, clickedColor, 1, ButtonBorderStyle.Solid, clickedColor, 1, ButtonBorderStyle.Solid);

            }
            else
            {
                ControlPaint.DrawBorder(e.Graphics, pics[0].ClientRectangle, PIC_UNCLICKED_COLOR, ButtonBorderStyle.Solid);
            }
        ...
        void Pics1_Paint(object sender, PaintEventArgs e)
        {
            if ((int)pics[1].Tag == 1)
            {
                ControlPaint.DrawBorder(e.Graphics, pics[1].ClientRectangle, PIC_CLICKED_COLOR, ButtonBorderStyle.Solid);

            }
            else
            {
                ControlPaint.DrawBorder(e.Graphics, pics[1].ClientRectangle, PIC_UNCLICKED_COLOR, ButtonBorderStyle.Solid);
            }
        ...
        void Pics2_Paint(object sender, PaintEventArgs e)
        {
            if ((int)pics[2].Tag == 1)
            {
                ControlPaint.DrawBorder(e.Graphics, pics[2].ClientRectangle, PIC_CLICKED_COLOR, ButtonBorderStyle.Solid);

            }
            else
            {
                ControlPaint.DrawBorder(e.Graphics, pics[2].ClientRectangle, PIC_UNCLICKED_COLOR, ButtonBorderStyle.Solid);
            }
        ...
        void Pics3_Paint(object sender, PaintEventArgs e)
        {
            if ((int)pics[3].Tag == 1)
            {
                ControlPaint.DrawBorder(e.Graphics, pics[3].ClientRectangle, PIC_CLICKED_COLOR, ButtonBorderStyle.Solid);

            }
            else
            {
                ControlPaint.DrawBorder(e.Graphics, pics[3].ClientRectangle, PIC_UNCLICKED_COLOR, ButtonBorderStyle.Solid);
            }
        ...
        /// <summary>
        /// 定时器1 每隔100ms刷新一次时间 通过timeLabel控件显示
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
